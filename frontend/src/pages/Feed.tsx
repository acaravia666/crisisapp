...
import { Clock, MapPin, Loader2, X, Search, Zap, Handshake, Package, ChevronRight } from 'lucide-react';
...
const URGENCY_STYLES: Record<string, { badge: string; glow: string; labelKey: string }> = {
  emergency: { badge: 'badge-emergency', glow: 'bg-urgency-emergency', labelKey: 'Emergency' },
  urgent:    { badge: 'badge-urgent',    glow: 'bg-urgency-urgent',    labelKey: 'Urgente'   },
  soon:      { badge: 'badge-soon',      glow: 'bg-urgency-soon',      labelKey: 'Pronto'    },
  normal:    { badge: 'badge-normal',    glow: 'bg-blue-500',          labelKey: 'Normal'    },
};

const URGENCY_LABELS_EN: Record<string, string> = {
  emergency: 'Emergency', urgent: 'Urgent', soon: 'Soon', normal: 'Normal',
};
const URGENCY_LABELS_ES: Record<string, string> = {
  emergency: 'Emergencia', urgent: 'Urgente', soon: 'Pronto', normal: 'Normal',
};
...
const Feed = () => {
  ...
  const filtered = useMemo(() => {
    let list = requests.filter(r => r.status !== 'fulfilled' && r.status !== 'cancelled');
  ...
  const urgencyLabels = language === 'es' ? URGENCY_LABELS_ES : URGENCY_LABELS_EN;
  ...
  return (
    <div className="flex flex-col h-full animate-slide-up pt-4 bg-bg-primary">
      ...
      <div className="flex justify-between items-center mb-4 px-4">
        <h1 className="text-2xl font-black italic tracking-tighter uppercase">{t('feed.title')}</h1>
        {hasFilter && (
          <button
      ...
      <div className="flex-1 overflow-y-auto pb-32 scrollbar-hide px-4">
          ...
          <section className="mb-10 space-y-4">
            <div className="flex justify-between items-center px-1">
              <h2 className="text-xs font-black uppercase tracking-[0.2em] text-muted flex items-center gap-2">
                <Handshake size={14} className="text-green-500" /> {t('feed.dealsInProgress')}
              </h2>
                ...
                >
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[10px] text-green-500 font-black uppercase tracking-widest mb-1">{t('feed.awaitingHandoff')}</p>
                      <h4 className="text-sm font-bold truncate mb-2">{tx.gear_name || '...'}</h4>
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-[10px] text-muted font-bold flex items-center gap-1">{t('feed.update')} <ChevronRight size={12}/></span>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        <section className="space-y-4">
          <div className="px-1 flex justify-between items-end">
            <h2 className="text-xs font-black uppercase tracking-[0.2em] text-muted flex items-center gap-2">
              <Zap size={14} className="text-accent-cyan" /> {t('feed.nearby')}
            </h2>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-20 bg-secondary/20 rounded-3xl border border-dashed border-white/5">
              <p className="text-secondary text-sm font-medium">
                {hasFilter ? t('feed.emptySearch') : t('feed.empty')}
              </p>
              {hasFilter && (
                  ...
                  >
                    ...
                    <div className="flex justify-between items-start mb-4 relative z-10">
                      <div>
                        <div className="flex items-center gap-2 mb-1.5">
                          ...
                          {isMyRequest && <span className="text-[9px] bg-accent-cyan text-black font-black px-2 py-0.5 rounded-full uppercase">{t('feed.dealInProgress')}</span>}
                        </div>
                        <div className="flex items-center gap-2 text-[10px] font-bold text-secondary uppercase tracking-widest">
                          <span className="flex items-center gap-1">
                            <MapPin size={10} className="text-accent-cyan" /> {req.users?.name || '—'}
                          </span>
                          {dist && <><span>•</span><span>{dist} {t('feed.away')}</span></>}
                        </div>
                      </div>
                      <span className={`badge ${style.badge} shadow-lg`}>{urgencyLabels[req.urgency] ?? req.urgency}</span>
                    </div>
                    ...
                    <div className="flex justify-between items-center pt-2 relative z-10">
                      ...
                      <div className="flex items-center gap-1 text-xs font-black text-white group">
                        {t('feed.details')} <ChevronRight size={14} className="group-hover:translate-x-1 transition-transform" />
                      </div>
                    </div>
                  </div>
                );
